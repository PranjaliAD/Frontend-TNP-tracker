...
function UploadCsvStudentsForm() {
  const [errorMessage, setErrorMessage] = useState('');
  ...
  const handleSubmit = async (event) => {
    ...
    const formData = new FormData();
    formData.append('student', selectedFile);

    try {
      const value = localStorage.getItem('adminData');
      const response = await axios.post(`https://placement-internship-tracker-backend.vercel.app/api/admins/student?adminemailId=${value}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
...
export default UploadCsvStudentsForm;
