...
function UploadCsvMentorForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    const formData = new FormData();
    formData.append('instructor', selectedFile);

    try {
      const value = localStorage.getItem('adminData');
      const response = await axios.post(`https://placement-internship-tracker-backend.vercel.app/api/admins/instructor?adminemailId=${value}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
