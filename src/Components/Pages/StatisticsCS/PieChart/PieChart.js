...
const PieChart = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const parsedData = rows.map(row => {
          ...
          return { Company_Name, Comp: parseInt(Comp), Sal_lpa: parseFloat(Sal_lpa) };
  ...
  data.forEach(entry => {
    ...
    companies[entry.Company_Name] += entry.Comp;
  });
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: 'Comp Students Placed by Company',
          ...
          tooltips: {
            callbacks: {
              label: (tooltipItem, data) => {
                ...
                return `${label}: ${value} Comp Students Placed`;
              },
            },
          },
        }}
